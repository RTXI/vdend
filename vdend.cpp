...
vdend::vdend(void) :
  DefaultGUIModel("vdend", ::vars, ::num_vars), dt(RT::System::getInstance()->getPeriod() * 1e-6), Gna_max(0.01),
      Gk_max(100.0), Gh_max(0.0), Gl_max(0.63), Vna(50.0), Vk(-80.0), Vh(-35.0), Vl(-51.0), rho_side(.034),
      rho_cent(.034), c_dend(34), Ioffset(0.0), twoway(0.0) {
  update(INIT);
  refresh();
}
...
void
vdend::update(DefaultGUIModel::update_flags_t flag) {
  switch (flag) {
    case INIT:
